...
import { GridItem, NodeType, SpaceType } from "./game-grid.types";
import { getTradeString, gridItemToColor } from "./game-grid.utils";
...
export const GameHex: React.FC<HexProps> = ({ size, xPos, yPos, gridItem }) => {
  ...
  const { scaledPoints, cordX, cordY } = useMemo(() => {
    const anchorX = 500;
    const anchorY = 100;
  ...
  function getText(item: GridItem) {
    if (item.type === NodeType.LAND) {
      ...
      return item.spaceType == SpaceType.EMPTY
        ? ""
        : getTradeString(item.spaceType);
    } else {
      return "";
    }
  }
